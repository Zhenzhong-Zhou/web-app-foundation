...
/**
 * Earliest expiry first: the order lot-tracked stock leaves a shelf, whether
 * issued to a production run (ADR-039) or shipped to a customer (ADR-041).
 *
 * Lives in the stock module because both callers draw on the same shelves by
 * the same rule, and neither owns it. The ordering also covers FIFO without a
 * setting: a lot with no expiry sorts after every dated one and then by when
 * it arrived, so non-perishable stock leaves oldest first.
 *
...
/**
 * A hand-picked allocation must add up to exactly what is being moved. Checked
 * in SQL, because summing decimal strings in JavaScript is the drift ADR-025
 * exists to prevent. Whether each lot belongs to the variant and holds enough
 * at the source is left to the stock service, which refuses both already.
 */
export async function assertPickedTotal(
  tx: Tx,
  picked: { lotId: string; quantity: string }[],
  expected: string,
  sku: string,
): Promise<void> {
  ...
  const result = await tx.execute(sql`
    select
      sum(v.q) = ${expected}::numeric as matches,
